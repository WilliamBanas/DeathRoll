...
const Lobby: React.FC = () => {
  ...
  useEffect(() => {
    if (socket && lobbyId) {
      ...
      socket.on("gameStarted", ({ currentTurn }) => {
        ...
        setGames((prevGames) => ({
          ...prevGames,
          [lobbyId]: { currentTurn, isActive: true, isFirstTurn: true, playerNumbers: {} } as Game, // Initialize isFirstTurn here
        }));
      });
  ...
  const startGame = () => {
    if (socket && lobbyId) {
      ...
      setGames((prevGames) => ({
        ...prevGames,
        [lobbyId]: { currentTurn: 0, isActive: true, isFirstTurn: true, playerNumbers: {} } as Game, // Ensure isFirstTurn is initialized
      }));
    }
  };
