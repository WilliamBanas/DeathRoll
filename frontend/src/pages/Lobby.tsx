...
const Lobby: React.FC = () => {
	...
	useEffect(() => {
		if (socket && lobbyId) {
			...
			socket.on("gameStarted", ({ currentTurn }) => {
				...
				setGames((prevGames) => ({
					...prevGames,
					[lobbyId]: { currentTurn, isActive: true } as Game, // Mark the game as active for all players
				}));
			});
	...
	const startGame = () => {
		if (socket && lobbyId) {
			// Ensure lobbyId is defined
			socket.emit("startGame", lobbyId);
			// Update games state when the game starts
			setGames((prevGames) => ({
				...prevGames,
				[lobbyId]: { currentTurn: 0, isActive: true } as Game, // Ensure TypeScript understands it's a Game type
			}));
		}
	};
	...
	return (
		<main className="flex flex-col items-center text-xl">
			...
			{lobbyId && !games[lobbyId]?.isActive && (
				<>
					{lobbyData?.host ===
					lobbyData?.players.find((player) => player.socketId === socket?.id)
						?.nickname ? (
